...
Before we start allocating all the resources we need to submit our jobs, we have to understand how the node
architecture works in Summit. Our goal is to provide insight to make better decisions in the construction of our particular
AMReX-Astro job scripts, and to explain how our code interacts with Summit. All the exposed information in this section
is a condensed version of the `Summit documentation guide <https://docs.olcf.ornl.gov/systems/summit_user_guide.html#job-launcher-jsrun>`_.
and should not replace it.

In Summit, a node is composed by two sockets: each one with 21 CPU cores, 3 GPU and 1 RAM memory bank.
The sockets are connected by an X-bus that allows communication among them, and each core may define up to 4 threads.
...
A resource set is a minimal set of CPU and GPU's, on which a certain number of MPI processes and OpenMP
...
Since each core supports up to 4 threads we could also allocate threads; however this option is not supported
in AMReX and we will not discuss it here. For now, we will fix only one thread through the whole execution
of our code. The next step is to determine the maximum number of resource sets that will fit into one node.

In Castro we construct each resource set with: 1 CPU physical core, 1 GPU, and only 1 MPI process.
The next step is to see how many resources sets fits into the node. Acording to the node architecture depicted in Figure 1,
...
To allocate the resource sets that we will use the command ``bsub`` in addition to some flags:

* ``-nnodes`` : allocates the number of nodes we need to run our code. Is important to perform the calculation
  described in the previous section to select the correct number of nodes in our setup.


* ``-W`` : allocates the walltime of the selected nodes. The format we use in Summit is [hours:]minutes, there is
  no room for seconds in Summit. The maximum walltime that we can request is 03:00 (three hours).


* ``-alloc_flags`` : allocates the maximum number of threads available per CPU core. By default the option is ``smt4`` that
...
* ``-J`` : defines the name of the allocation. The value ``%J`` correspond to the allocation ID number.

* ``-o`` : defines the **output name that contains the standard output stream**, after running all the jobs inside the requested
  allocation.

* ``-e`` : defines the **output file name containing the standard error stream**, similar to the ``-o`` flag. If ``-e`` is not supplied, then
  the ``-o`` option is assumed by default.

* ``-q`` : defines the queue on which our application will run. There are several options, however, we alternate
...
* ``-Is`` : flags for interactive job followed by the shell name. The Unix bash shell option is ``/bin/bash``. This flag is very useful
...
from the 480 resources available, 398 resources will remain idle until the two working processes sweep the entire Level 0. Therefore,
is important, if possible, to keep the number of boxes on each level balanced to maximize the use of the allocated resources.
...
after a certain number of timesteps, the code creates checkpoint files of the form ``chk*``.
This checkpoint files can be read by our executable and run from the simulation time where the checkpoint was
created. This is implemented as follows:
